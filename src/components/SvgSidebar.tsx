...
const SvgSidebar = ({ svgs, selectedId, onSelect, onUpload }: SvgSidebarProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <div className="px-4 py-3 flex items-center justify-between" style={{ background: "hsl(var(--titlebar-bg))", borderBottom: "1px solid hsl(var(--border))" }}>
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-3">
        {isDragOver ? (
          <div
            className="flex flex-col items-center justify-center gap-2 h-40 rounded-lg border-2 border-dashed transition-all"
              ...
              return (
                <Tooltip key={svg.id} title={svg.name} mouseEnterDelay={0.4}>
                  <div
                    ...
                    className="aspect-square rounded flex items-center justify-center cursor-pointer transition-all duration-100"
                    style={{
                  ...
                  >
                    <div className="w-5 h-5 [&_svg]:fill-current" style={{ color: "hsl(var(--muted-foreground))" }} dangerouslySetInnerHTML={{ __html: svg.svg }} />
                  </div>
                </Tooltip>
              );
            })}
        ...
        {!isDragOver && filtered.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-xs mt-2" style={{ color: "hsl(var(--muted-foreground))" }}>No icons found</p>
          </div>
        )}
      </div>
    </div>
